...
def dersleri_listele_gui():
    ...
    def guncelle_goster(row_idx, ders_id, mevcut_ad, mevcut_saat):
        ...
        ctk.CTkButton(form, text="Kaydet", command=kaydet, width=100).grid(row=0, column=5, padx=10)
        ctk.CTkButton(form, text="İptal", command=iptal_et, width=80, fg_color="gray", hover_color="darkgray").grid(row=1, column=6, padx=(0, 10), pady=5)
    ...
    for i, ders in enumerate(dersler, start=1):
        ...
        ctk.CTkLabel(row_frame, text=str(ders_id), font=("Arial", 15), width=80, anchor="w").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ctk.CTkLabel(row_frame, text=ders_adi, font=("Arial", 15), width=200, anchor="w").grid(row=0, column=1, padx=10, pady=5, sticky="w")
        ctk.CTkLabel(row_frame, text=ders_saati, font=("Arial", 15), width=100, anchor="w").grid(row=0, column=2, padx=10, pady=5, sticky="w")

        ctk.CTkButton(row_frame, text="Sil", fg_color="red", hover_color="#8B0000",
                      width=60, height=28, font=("Arial", 12),
                      command=lambda d_id=ders_id, f=row_frame: sil_ders(d_id, f)).grid(row=0, column=3, padx=(5, 5), pady=5)

        ctk.CTkButton(row_frame, text="Güncelle", fg_color="#FFA500", hover_color="#cc8400",
